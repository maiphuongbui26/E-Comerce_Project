...
const TShirts = () => {
  ...
  useEffect(() => {
    const initializePage = async () => {
      ...
      // Find the t-shirt category and set it as selected
      const tshirtCategory = categories?.find(cat => 
        cat.TenDanhMuc.toLowerCase().includes('áo thun')
      );
      if (tshirtCategory) {
        setSelectedCategories([tshirtCategory.id]);
      }
    };
  ...
  const filteredProducts = products?.filter(product => {
    const isTShirtProduct = selectedCategories.length > 0 
      ? selectedCategories.includes(product.DanhMucSanPham.id)
      : product.DanhMucSanPham.TenDanhMuc.toLowerCase().includes('áo thun');

    if (selectedPriceRanges.length > 0) {
      const price = product.GiaSanPham;
      return isTShirtProduct && selectedPriceRanges.some(range => {
        switch (range) {
  ...
  return (
    <>
      <Box sx={{ maxWidth: "1240px", margin: "0 auto" }}>
        ...
        >
          <span>{sortedProducts?.length || 0}</span> sản phẩm <span>Áo Thun</span>
        </Typography>
