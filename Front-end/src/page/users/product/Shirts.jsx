...
const Shirts = () => {
  ...
  useEffect(() => {
    const initializePage = async () => {
      ...
      // Find the shirt category and set it as selected
      const shirtCategory = categories?.find(cat => 
        cat.TenDanhMuc.toLowerCase().includes('áo')
      );
      if (shirtCategory) {
        setSelectedCategories([shirtCategory.id]);
      }
    };
  ...
  // Update filteredProducts to always filter by shirt category
  const filteredProducts = products?.filter(product => {
    const isShirtProduct = selectedCategories.length > 0 
      ? selectedCategories.includes(product.DanhMucSanPham.id)
      : product.DanhMucSanPham.TenDanhMuc.toLowerCase().includes('áo');

    if (selectedPriceRanges.length > 0) {
      const price = product.GiaSanPham;
      return isShirtProduct && selectedPriceRanges.some(range => {
        switch (range) {
    ...
    <>
      <Box sx={{ maxWidth: "1240px", margin: "0 auto" }}>
        ...
        <Typography variant="h4" sx={{ marginTop: "50px", fontSize: "18px", color: "#303030", fontWeight: "600" }}>
          <span>{sortedProducts?.length || 0}</span> sản phẩm <span>Áo</span>
        </Typography>
