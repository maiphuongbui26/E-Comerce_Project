...
const Jackets = () => {
  ...
  useEffect(() => {
    const initializePage = async () => {
      ...
      // Find the jacket category and set it as selected
      const jacketCategory = categories?.find(cat => 
        cat.TenDanhMuc.toLowerCase().includes('jacket')
      );
      if (jacketCategory) {
        setSelectedCategories([jacketCategory.id]);
      }
    };
  ...
  // Update filteredProducts to always filter by jacket category
  const filteredProducts = products?.filter(product => {
    // Always filter for jacket products
    const isJacketProduct = selectedCategories.length > 0 
      ? selectedCategories.includes(product.DanhMucSanPham.id)
      : product.DanhMucSanPham.TenDanhMuc.toLowerCase().includes('jacket');

    // Apply price filter if selected
    if (selectedPriceRanges.length > 0) {
      const price = product.GiaSanPham;
      return isJacketProduct && selectedPriceRanges.some(range => {
        switch (range) {
  ...
  return (
    <>
      <Box sx={{ maxWidth: "1240px", margin: "0 auto" }}>
        ...
        >
          <span>{sortedProducts?.length || 0}</span> sản phẩm <span>Jacket</span>
        </Typography>
