...
const EditPromotion = () => {
  ...
  const { id } = useParams();
  const { discounts, handleUpdateDiscount, handleFetchDiscounts, isLoading } = useDiscount();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    await handleUpdateDiscount(id, formData);
    ...
    <>
      <Box sx={{ p: 2, bgcolor: '#fff', borderRadius: '4px 4px 0 0', borderBottom: '1px solid #e0e0e0' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <EditIcon />
          <Typography variant="h5">Chỉnh sửa mã giảm giá</Typography>
        </Box>
      </Box>
